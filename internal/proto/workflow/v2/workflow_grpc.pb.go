...
type WorkflowServiceClient interface {
	...
	StreamWorkflows(ctx context.Context, in *StreamWorkflowsRequest, opts ...grpc.CallOption) (WorkflowService_StreamWorkflowsClient, error)
...
func (c *workflowServiceClient) StreamWorkflows(ctx context.Context, in *StreamWorkflowsRequest, opts ...grpc.CallOption) (WorkflowService_StreamWorkflowsClient, error) {
	stream, err := c.cc.NewStream(ctx, &WorkflowService_ServiceDesc.Streams[0], "/internal.proto.workflow.v2.WorkflowService/StreamWorkflows", opts...)
	if err != nil {
		return nil, err
	}
	x := &workflowServiceStreamWorkflowsClient{stream}
...
type WorkflowService_StreamWorkflowsClient interface {
	Recv() (*StreamWorkflowsResponse, error)
	grpc.ClientStream
}

type workflowServiceStreamWorkflowsClient struct {
	grpc.ClientStream
}

func (x *workflowServiceStreamWorkflowsClient) Recv() (*StreamWorkflowsResponse, error) {
	m := new(StreamWorkflowsResponse)
...
type WorkflowServiceServer interface {
	// Stream creates a stream that will receive workflows intended for the agent identified
	// by the StreamWorkflowsRequest.agent_id.
	StreamWorkflows(*StreamWorkflowsRequest, WorkflowService_StreamWorkflowsServer) error
	// PublishEvent publishes a workflow event.
...
func (UnimplementedWorkflowServiceServer) StreamWorkflows(*StreamWorkflowsRequest, WorkflowService_StreamWorkflowsServer) error {
	return status.Errorf(codes.Unimplemented, "method StreamWorkflows not implemented")
}
...
func _WorkflowService_StreamWorkflows_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamWorkflowsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(WorkflowServiceServer).StreamWorkflows(m, &workflowServiceStreamWorkflowsServer{stream})
}

type WorkflowService_StreamWorkflowsServer interface {
	Send(*StreamWorkflowsResponse) error
	grpc.ServerStream
}

type workflowServiceStreamWorkflowsServer struct {
	grpc.ServerStream
}

func (x *workflowServiceStreamWorkflowsServer) Send(m *StreamWorkflowsResponse) error {
...
var WorkflowService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "StreamWorkflows",
			Handler:       _WorkflowService_StreamWorkflows_Handler,
			ServerStreams: true,
		},
	},
