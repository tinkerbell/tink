...
func TestTwoSuccessfulWorkflows(t *testing.T) {
	...
	_, err = machine.Exec(ctx, "docker tag hello-world 192.168.1.1/hello-world-again")
	if err != nil {
		t.Fatal(err)
	}

	_, err = machine.Exec(ctx, "docker push 192.168.1.1/hello-world-again")
	if err != nil {
		t.Fatal(err)
	}
	...
	templateID, err = registerTemplates(ctx, "hello-world-again.tmpl")
	if err != nil {
		t.Fatal(err)
	}
	...
	for iii := 0; iii < 30; iii++ {
		...
		for event, err := events.Recv(); err == nil && event != nil; event, err = events.Recv() {
			if event.ActionName == "hello_world_again" && event.ActionStatus == workflow.ActionState_ACTION_SUCCESS {
				t.Logf("action %s SUCCESSFUL as expected", event.ActionName)
				return
			}
		}
